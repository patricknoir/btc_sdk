...
class ScriptOpAdd extends ScriptOperation {
  ...
  int run(Stack<ScriptElement> stack) {
    if(stack.length >= 2) {
      final element1 = stack.pop() as ScriptData;
      final element2 = stack.pop() as ScriptData;
...
class ScriptOpCheckSig extends ScriptOperation {
  ...
  int run(Stack<ScriptElement> stack) {
    if(stack.length >= 2) {
      final pubKey = stack.pop() as ScriptData;
      final sig = stack.pop() as ScriptData;
      final sigHash = SignedHash.fromDER(pubKey.data, sig.data);
